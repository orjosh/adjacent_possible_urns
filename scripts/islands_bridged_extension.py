...
class IslandsBridgedMdlExt(AdjPosModel):
    """
    ...
    appear in the network, unlike in the original model.

    In this version of the islands extension, if a new caller-receiver pair is created, the
    caller also obtains an ID from anywhere in the network chosen proportionally by size.
    """
    ...
    def time_step(self):
        ...
        if r < self.prob_core and self.n_core < self.max_core:
            ...
            # create "bridge" by connecting to a proportionally chosen urn
            urn_id = caller.ID
            while urn_id == caller.ID:
                # want to pick another ID that isn't our caller
                urn_id = self._get_calling_urn()

            caller.add_contact(urn_id)
            self.prop_choice.append(caller.ID)
